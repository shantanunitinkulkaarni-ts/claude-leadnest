    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:${THEME.card};border-radius:16px;overflow:hidden;border:1px solid ${THEME.border};box-shadow:0 1px 3px rgba(20,22,27,0.04);">
        ...
        <tr><td style="background:linear-gradient(135deg,${THEME.primary},${THEME.accent});padding:22px 32px;">
          <span style="color:#ffffff;font-size:19px;font-weight:700;letter-spacing:-0.02em;">Convorian</span>
        </td></tr>
        <!-- Body -->
        ...
        <tr><td style="padding:22px 32px;border-top:1px solid ${THEME.border};font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
          <p style="margin:0 0 6px;color:${THEME.sub};font-size:13px;line-height:1.5;">
            <strong style="color:${THEME.ink};">Convorian</strong> — your AI WhatsApp assistant for real estate.
          </p>
          <p style="margin:0;color:${THEME.faint};font-size:12px;line-height:1.5;">
            ...
            <br/>You're receiving this because you have a Convorian account.
          </p>
        </td></tr>
      </table>
      <p style="margin:16px 0 0;color:${THEME.faint};font-size:11px;font-family:-apple-system,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">© 2026 Convorian</p>
    </td></tr>
  </table>
</body>
</html>`
...
export async function sendWelcomeEmail(to: string, name?: string) {
  const hi = name ? `Welcome, ${firstName(name)} 👋` : 'Welcome to Convorian 👋'
  return sendEmail({
    to,
    subject: 'Welcome to Convorian — let’s set up your AI assistant',
    replyTo: SUPPORT,
    html: layout({
      preheader: 'Your AI WhatsApp assistant is ready. Add your first lead in 2 minutes.',
      body: `
        ${h1(hi)}
        ${p('Your account is live. Convorian’s AI now works 24/7 — answering leads, qualifying them, and booking site visits on WhatsApp, even while you sleep.')}
...
export const NURTURE_SEQUENCE: NurtureStep[] = [
  {
    ...
    send: (to, name) =>
      sendEmail({
        ...
        replyTo: SUPPORT,
        html: layout({
          preheader: 'Add one lead and Convorian starts the conversation for you.',
          body: `
            ${h1(`Hi ${firstName(name)}, let’s get your first win`)}
            ${p('You signed up yesterday — the single best thing you can do now is add one real lead. Convorian instantly starts a natural WhatsApp conversation, qualifies them, and nudges toward a site visit.')}
  ...
  },
  {
    ...
    send: (to, name) =>
      sendEmail({
        to,
        subject: '3 ways agents close more with Convorian',
        replyTo: SUPPORT,
        html: layout({
          preheader: 'Small setup tweaks that noticeably lift conversions.',
          body: `
            ${h1('Get more out of your assistant')}
            ${p(`A few things, ${firstName(name)}, that the top agents on Convorian do:`)}
            ${infoBox(`
      ...
      sendEmail({
        ...
        html: layout({
          ...
          body: `
            ...
            ${p(`Hi ${firstName(name)} — you’ve seen what Convorian can do across ${ctx.leadsCount} lead${ctx.leadsCount === 1 ? '' : 's'}. To keep your assistant running without interruption, lock in a plan.`)}
      ...
      sendEmail({
        ...
        html: layout({
          ...
          body: `
            ...
            ${p(`${firstName(name)}, most deals aren’t lost at the pitch — they’re lost in the silence after the first message. That’s exactly the gap Convorian fills: it never forgets, never gets tired, and follows up at the right time, every time.`)}
      ...
      sendEmail({
        ...
        html: layout({
          ...
          body: `
            ...
            ${button('Continue with Convorian', `${APP_URL}/dashboard`)}
            ${p('And if something’s holding you back, reply and tell me — I read every response personally. — Team Convorian')}
          `,
        }),
      }),
  },
