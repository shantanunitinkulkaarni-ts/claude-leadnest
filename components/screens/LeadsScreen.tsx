...
export default function LeadsScreen({ agentId }: Props) {
  ...
  const fetchLeads = () => {
    fetch('/api/leads?agent_id=' + agentId)
      .then(async r => {
        ...
        if (!r.ok) { setFetchError(d.error || `Error ${r.status}`); return }
        setFetchError(null)
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      if (!res.ok) { setPinErr(data.error || 'Incorrect PIN'); return }
      setIsUnlocked(true)
  ...
  const handleDrop = async (e: React.DragEvent, colStatus: string) => {
    ...
    if (colStatus === 'Closed') {
      ...
      if (!choice) { setDraggedId(null); return }
    ...
    // Real update — revert on any failure, including non-2xx HTTP responses
    try {
      ...
      fetchLeads() // Revert UI to match actual DB state
    }
    setDraggedId(null)
  }
  ...
  const handleSaveLead = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLeadName(''); setLeadPhone(''); setLeadConsent(false)
      fetchLeads()
  ...
  const handleBulkUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(e.target.files[0], {
      ...
      complete: async (results) => {
        ...
        setBulkProgress(0)
        const payloadArray = rows
          .map(row => ({ agent_id: agentId, name: row['Name'] || 'Unknown', phone: row['Phone'] || '', source: row['Source'] || 'Bulk Upload', status: 'new', temperature: 'new', ai_score: 0 }))
          .filter(l => l.phone !== '')
        try {
          const res = await fetch('/api/leads', {
            method: 'POST',
        ...
        setTimeout(() => {
          setIsBulkUploading(false); setShowBulkModal(false); fetchLeads()
  ...
  const grouped = COLUMNS.reduce((acc, col) => {
    acc[col] = leads.filter(l => (STATUS_MAP[col] || []).includes(l.status || 'new'))
    return acc
  }, {} as Record<string, any[]>)

  const getScoreColor = (score: number) => {
    if (score >= 8) return { bg: '#EEF0FE', c: '#4338CA' }
    if (score >= 5) return { bg: '#FEF9E7', c: '#7A5200' }
    return { bg: '#EEF4FC', c: '#0F3D6E' }
  }

  return (
    <div className="mobile-scroll" style={{ padding: '16px', height: '100%', overflowY: 'auto', position: 'relative' }}>
      <style>{`.draggable-card:hover { transform: translateY(-2px); box-shadow: 0 4px 8px rgba(0,0,0,0.05) !important; } .locked-card:hover { filter: brightness(0.97); }`}</style>
      ...
      {fetchError && (
        <div style={{ background: '#FDF0F0', border: '1px solid rgba(192,57,43,0.2)', color: '#8B1A1A', padding: '10px 16px', borderRadius: 8, marginBottom: 16, fontSize: 13 }}>
          ...
          {(fetchError.includes('401') || fetchError.includes('403')) ? ' — Please refresh the page or log out and log back in.' : ''}
        </div>
      )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#6B6860', marginTop: 4 }}>🤖 AI automatically moves leads based on intent and qualification score.</div>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => setShowBulkModal(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 8, background: '#fff', color: '#15161B', border: '1px solid rgba(26,25,22,0.18)', cursor: 'pointer', fontSize: 13, fontWeight: 500, fontFamily: 'inherit' }}>Bulk Upload CSV</button>
          <button data-tour="add-lead" onClick={() => { setShowAddModal(true); setAddError(null) }} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 8, background: '#15161B', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 500, fontFamily: 'inherit' }}>+ Add Lead</button>
          <button onClick={() => !isUnlocked ? setShowPinModal(true) : setIsUnlocked(false)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 8, background: isUnlocked ? '#FEF9E7' : '#F4F3EE', color: isUnlocked ? '#7A5200' : '#6B6860', border: isUnlocked ? '1px solid rgba(183,119,13,0.3)' : '1px solid transparent', cursor: 'pointer', fontSize: 13, fontWeight: 500, fontFamily: 'inherit' }}>
      ...
      {!loading && !fetchError && leads.length === 0 && (
        <div style={{ textAlign: 'center', padding: '80px 20px', color: '#9E9B92' }}>
          ...
          <div style={{ fontSize: 13, marginBottom: 24 }}>Add your first lead manually, or leads will appear automatically when someone messages you on WhatsApp.</div>
          <button onClick={() => { setShowAddModal(true); setAddError(null) }} style={{ padding: '10px 20px', borderRadius: 8, background: '#15161B', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 500, fontFamily: 'inherit' }}>+ Add your first lead</button>
        </div>
      )}
      ...
      {!(loading && leads.length === 0) && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, alignItems: 'start' }}>
          {COLUMNS.map(col => {
            ...
            return (
              <div key={col} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, col)} style={{ background: '#F4F3EE', border: isUnlocked ? '1px dashed rgba(79,70,229,0.4)' : '1px solid rgba(26,25,22,0.08)', borderRadius: 14, padding: 12, minHeight: 'calc(100vh - 180px)', transition: 'border 0.2s' }}>
                ...
                {colLeads.map(lead => {
                  ...
                  return (
                    <div key={lead.id} className={isUnlocked ? "draggable-card" : "locked-card"} draggable={isUnlocked} onDragStart={(e) => handleDragStart(e, lead.id)} onDragEnd={handleDragEnd} style={{ background: '#fff', border: '1px solid rgba(26,25,22,0.08)', borderRadius: 10, padding: '14px 16px', marginBottom: 8, cursor: isUnlocked ? 'grab' : 'default', transition: 'all 0.15s', boxShadow: '0 1px 2px rgba(0,0,0,0.02)' }}>
                      ...
                      <div style={{ fontSize: 11, color: '#9E9B92', marginTop: 4 }}>{lead.intent || 'Unknown intent'}</div>
                      {lead.budget_max && <div style={{ fontSize: 11, color: '#6B6860', marginTop: 6 }}>Up to {formatINR(lead.budget_max)}{lead.intent === 'rent' ? '/mo' : ''}</div>}
                    </div>
                  )
                })}
                {colLeads.length === 0 && <div style={{ textAlign: 'center', padding: '20px 0', fontSize: 12, color: 'rgba(107,104,96,0.5)', border: '1px dashed rgba(26,25,22,0.1)', borderRadius: 8 }}>Drop leads here</div>}
              </div>
            )
          })}
        </div>
      ...
      {/* PIN Modal — server-side verification */}
      {showPinModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(26,25,22,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(3px)' }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: '24px 30px', width: 320, boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ fontSize: 12, color: '#9E9B92', marginBottom: 20 }}>Enter your master PIN to manually move leads.</div>
            {pinErr && <div style={{ background: '#FDF0F0', color: '#8B1A1A', padding: '8px 12px', borderRadius: 8, fontSize: 12, marginBottom: 12 }}>⚠️ {pinErr}</div>}
            <input type="password" value={pinInput} onChange={e => setPinInput(e.target.value)} placeholder="Enter PIN" onKeyDown={e => e.key === 'Enter' && handleUnlock()} autoFocus style={{ width: '100%', padding: '10px 14px', borderRadius: 8, border: '1px solid rgba(26,25,22,0.18)', fontSize: 14, fontFamily: 'inherit', marginBottom: 16, outline: 'none' }} />
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button onClick={() => { setShowPinModal(false); setPinInput(''); setPinErr('') }} disabled={pinLoading} style={{ padding: '8px 16px', borderRadius: 8, background: '#F4F3EE', color: '#6B6860', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 500, fontFamily: 'inherit' }}>Cancel</button>
              <button onClick={handleUnlock} disabled={pinLoading || !pinInput} style={{ padding: '8px 16px', borderRadius: 8, background: '#15161B', color: '#fff', border: 'none', cursor: pinLoading ? 'wait' : 'pointer', fontSize: 13, fontWeight: 500, fontFamily: 'inherit', opacity: pinLoading ? 0.7 : 1 }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(26,25,22,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(4px)' }}>
          <form onSubmit={handleSaveLead} style={{ background: '#fff', borderRadius: 16, width: 450, display: 'flex', flexDirection: 'column', boxShadow: '0 20px 50px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              {addError && <div style={{ background: '#FDF0F0', color: '#8B1A1A', padding: '10px 14px', borderRadius: 8, fontSize: 13 }}>⚠️ {addError}</div>}
              <div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(26,25,22,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: '#fff', borderRadius: 16, width: 450, boxShadow: '0 20px 50px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
              {!isBulkUploading ? (
                <>
                  <div style={{ fontSize: 13, color: '#6B6860', lineHeight: 1.5 }}>Upload a CSV file to import your existing leads pipeline. Our AI will automatically begin engaging them if they send a message.</div>
                  <button onClick={downloadTemplate} style={{ padding: '8px 16px', borderRadius: 8, background: '#F4F3EE', color: '#15161B', border: '1px solid rgba(26,25,22,0.08)', cursor: 'pointer', fontSize: 13, fontWeight: 500, fontFamily: 'inherit', alignSelf: 'flex-start' }}>⬇️ Download CSV Template</button>
                  <div onClick={() => bulkFileInputRef.current?.click()} style={{ border: '1px dashed rgba(26,25,22,0.2)', borderRadius: 8, padding: '30px', textAlign: 'center', cursor: 'pointer', background: '#fff', marginTop: 10 }}>
