...
function Nav() {
  ...
  return (
    <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: '#FAFAF7', borderBottom: `1px solid ${G.border}`, backdropFilter: 'blur(12px)', backgroundClip: 'padding-box' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '14px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 28, height: 28, borderRadius: 8, background: `linear-gradient(135deg, ${G.green}, #1A6B4A)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: '#fff' }}>🏠</div>
          <span style={{ fontSize: 16, fontWeight: 600, color: G.dark, letterSpacing: '-0.01em' }}>LeadNest</span>
        </div>
        <div style={{ display: 'flex', gap: 32, alignItems: 'center' }}>
          ...
          <a href="#demo" style={{ padding: '8px 16px', borderRadius: 8, background: G.green, color: '#fff', fontSize: 13, fontWeight: 600, textDecoration: 'none', cursor: 'pointer' }}>
            Get Started
          </a>
        </div>
      </div>
...
function Hero() {
  return (
    <section style={{ background: '#FAFAF7', position: 'relative', overflow: 'hidden', paddingTop: 60, paddingBottom: 80 }}>
      ...
      <div style={{ position: 'absolute', width: 300, height: 300, background: 'radial-gradient(circle, rgba(184,149,90,0.15) 0%, transparent 70%)', bottom: -50, right: -50, pointerEvents: 'none' }} />
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 32px', position: 'relative', zIndex: 1, textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 18, color: G.muted, lineHeight: 1.6, maxWidth: 700, margin: '0 auto 32px', fontWeight: 400 }}>
          LeadNest is an AI sales engine built for real estate agents. Your bot answers leads 24/7, qualifies them, books site visits, and tracks ROI. Close deals on autopilot.
        </p>

        <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 32 }}>
          <a href="#demo" style={{ padding: '14px 32px', borderRadius: 12, background: G.green, color: '#fff', fontSize: 14, fontWeight: 600, textDecoration: 'none', cursor: 'pointer', border: 'none' }}>
            Start Free Trial
          </a>
    ...
    <section style={{ paddingTop: 80, paddingBottom: 80, background: '#fff' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 24 }}>
          {issues.map((issue, i) => (
            <div key={i} style={{ padding: 28, borderRadius: 12, border: `1px solid ${G.border}`, background: '#FAFAF7' }}>
...
function Solution() {
  ...
  return (
    <section style={{ paddingTop: 80, paddingBottom: 80, background: '#FAFAF7' }} id="features">
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 32px' }}>
        <div style={{ textAlign: 'center', marginBottom: 60 }}>
          <h2 style={{ fontSize: 40, fontWeight: 700, color: G.dark, letterSpacing: '-0.02em', marginBottom: 16 }}>How LeadNest Works</h2>
  ...
  return (
    <section style={{ paddingTop: 80, paddingBottom: 80, background: '#fff' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 32px' }}>
        ...
        <div style={{ marginTop: 60, padding: '40px', borderRadius: 12, background: '#1A1916', color: '#fff' }}>
...
function Pricing() {
  return (
    <section style={{ paddingTop: 80, paddingBottom: 80, background: '#FAFAF7' }} id="pricing">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28 }}>
          {[
            ...
            <div key={i} style={{ padding: 32, borderRadius: 16, border: plan.highlight ? `2px solid ${G.green}` : `1px solid ${G.border}`, background: plan.highlight ? G.greenLight : '#fff', position: 'relative' }}>
              ...
              <button style={{ width: '100%', padding: '12px 24px', borderRadius: 12, background: plan.highlight ? G.green : '#fff', color: plan.highlight ? '#fff' : G.dark, border: plan.highlight ? 'none' : `1px solid ${G.border}`, fontSize: 14, fontWeight: 600, cursor: 'pointer', marginBottom: 24 }}>
                Start Free Trial
              </button>
...
function CTA() {
  return (
    <section style={{ paddingTop: 80, paddingBottom: 80, background: '#1A1916', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', width: 400, height: 400, background: `radial-gradient(circle, rgba(46,139,95,0.15) 0%, transparent 70%)`, top: -100, right: -100 }} />
      ...
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '0 32px', position: 'relative', zIndex: 1, textAlign: 'center' }}>
        <h2 style={{ fontSize: 42, fontWeight: 700, color: '#fff', letterSpacing: '-0.02em', marginBottom: 20, lineHeight: 1.2 }}>
          Your first deal pays for a year of LeadNest
        </h2>
        <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.6)', marginBottom: 32, lineHeight: 1.6 }}>
          Stop losing leads to time delays. Start closing deals on autopilot. Join 50+ agents already using LeadNest.
        </p>

        <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
          <button id="demo" style={{ padding: '14px 32px', borderRadius: 12, background: G.green, color: '#fff', fontSize: 14, fontWeight: 600, border: 'none', cursor: 'pointer' }}>
            Start Free Trial (No Card)
          </button>
          <a href="https://wa.me/919876543210?text=Hi%20LeadNest!%20I%20want%20to%20schedule%20a%20demo" target="_blank" style={{ padding: '14px 32px', borderRadius: 12, background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: '#fff', fontSize: 14, fontWeight: 600, textDecoration: 'none', cursor: 'pointer' }}>
            Chat on WhatsApp
          </a>
        </div>
  ...
  return (
    <footer style={{ padding: '32px', borderTop: `1px solid ${G.border}`, background: '#fff', textAlign: 'center' }}>
      <div style={{ fontSize: 12, color: G.muted }}>
        © 2026 LeadNest. Built by real estate agents, for real estate agents. | <a href="#" style={{ color: G.green, textDecoration: 'none' }}>Terms</a> | <a href="#" style={{ color: G.green, textDecoration: 'none' }}>Privacy</a>
      </div>
    </footer>
  )
}
